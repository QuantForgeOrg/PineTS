...
describe('Matrix Namespace', () => {
    it('should handle basic matrix operations correctly', async () => {
        ...
        const sourceCode = (context: any) => {
            ...
            // Add/Remove
            // Wrap array in another array to prevent it being interpreted as a time series history of scalars [1, 2, 3] -> 3
            matrix.add_row(m1, 2, [[1, 2, 3]]); 
            const rows2 = matrix.rows(m1); // 3
            const val6 = matrix.get(m1, 2, 0);
            
            matrix.add_col(m1, 3, [[9, 9, 9]]);
    ...
    it('should handle matrix transformations', async () => {
        ...
        const sourceCode = (context: any) => {
            ...
            const rev_val = matrix.get(m_rev, 0, 0); // 4
        ...
        expect(last(result.rev_val)).toBe(4);
    });
});
